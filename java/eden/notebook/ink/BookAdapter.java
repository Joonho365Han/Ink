...
public class BookAdapter extends RecyclerView.Adapter<BookAdapter.NoteHolder> {
    ...
    public void onBindViewHolder(NoteHolder noteHolder, int index) {
        ...
        if (index < columns){ // The very first holder is a placeholder for the recyclerview. It makes the notes show up a little below from the Action Bar.
            noteHolder.mBackground.setVisibility(View.INVISIBLE); return; //Make it invisible and just stop. No need to customize rest of holder.
        } else if (noteHolder.mBackground.getVisibility() == View.INVISIBLE){ // Other indices: make sure it is visible.
        ...
        if (editDate.equals(date)) noteHolder.mEdited.setText(context.getResources().getString(R.string.today) + editedAt.substring(editDate.length()).trim());
        ...
        //Not a group delete mode NOR a candidate for deletion (= show color category)
                ...
                if (allStars.get(index)==1)
                    if (allColors.get(index)==0) noteHolder.mStar.setBackgroundResource(R.drawable.ic_stars_white_36dp_alternative);
    ...
    class NoteHolder extends RecyclerView.ViewHolder{ //Don't make it private since it must access the upper class' variables.
        ...
        public NoteHolder(View itemView) {
            ...
            mPhotoTint = (FrameLayout) itemView.findViewById(R.id.photo_tint);
                ...
                public void onClick(View v) {
                    ...
                    //Group-delete mode makes notes go checked or unchecked.
                                    ...
                                    //Uncheck
                                                ...
                                                if (colorIndex == 8) { //Image background.
                                                    try {
                                                        mBackground.setBackground(new BitmapDrawable(context.getResources(), BitmapFactory
                                                                .decodeFile(new File(context.getFilesDir(), title + "AG5463#$1!#$&")
                                                                                .getAbsolutePath(),
                                                                        new BitmapFactory.Options())/*<-bitmap image*/ ) /*<-drawable(int id)*/ );
                                                ...
                                                if (allStars.get(adapterIndex)==1)
                                                    if (allColors.get(adapterIndex)==0) mStar.setBackgroundResource(R.drawable.ic_stars_white_36dp_alternative);
